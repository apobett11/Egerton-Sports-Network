        ...
        <EmptyState
          ...
          action={
            <button
              ...
              className="min-h-[44px] px-5 py-2.5 bg-amber-600 hover:bg-amber-500 text-slate-950 font-black text-xs rounded-xl shadow-md cursor-pointer"
            >
              Go to My Matches
            </button>
          }
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      ...
      <section className="bg-[#12171B] border border-slate-800/80 rounded-2xl p-5 sm:p-6 shadow-xl space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-11 items-center gap-4 bg-[#0B0F12] p-6 rounded-xl border border-slate-800/80">
          <div className="md:col-span-5 flex items-center justify-start md:justify-end gap-4 text-left md:text-right">
            <div>
              ...
              <span className="text-xs text-slate-500 font-medium">Home Team</span>
            </div>
            <div className="w-14 h-14 rounded-xl bg-[#171D22] p-2 border border-slate-800 flex items-center justify-center flex-shrink-0">
              <img src={selectedFixture.teamA.logo} alt="" className="w-full h-full object-contain" />
            </div>
          </div>

          <div className="md:col-span-1 text-center font-mono font-black text-2xl text-amber-500 py-2 md:py-0">
          ...
          <div className="md:col-span-5 flex items-center justify-start gap-4">
            ...
            <div>
              ...
              <span className="text-xs text-slate-500 font-medium">Away Team</span>
            </div>
          </div>
        </div>

        {/* Match Metadata Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs bg-[#171D22] p-4 rounded-xl border border-slate-800/90 shadow-md">
        ...
        {(isFinished || isCancelled || goals.length > 0 || yellowCards.length > 0) && (
          <div className="space-y-4 bg-[#171D22] p-5 rounded-xl border border-slate-800/90 shadow-md">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <span className="px-2.5 py-0.5 rounded-md text-[10px] font-extrabold bg-amber-600/20 border border-amber-600/40 text-amber-500">
                VERIFIED REPORT
              </span>
            </div>
            ...
            <div className="space-y-2 text-xs">
              ...
              {goals.length === 0 ? (
                ...
                <div className="space-y-1.5">
                  {goals.map((g) => (
                    <div key={g.id} className="p-2.5 bg-[#0B0F12] rounded-lg border border-slate-800 flex items-center justify-between">
            ...
            <div className="space-y-2 text-xs pt-2">
              ...
              {yellowCards.length === 0 ? (
                ...
                <div className="space-y-1.5">
                  {yellowCards.map((c) => (
                    <div key={c.id} className="p-2.5 bg-[#0B0F12] rounded-lg border border-slate-800 flex items-center justify-between">
            ...
            <div className="space-y-2 text-xs pt-2">
              ...
              {injuries.length === 0 ? (
                ...
                <div className="space-y-1.5">
                  {injuries.map((i) => (
                    <div key={i.id} className="p-2.5 bg-[#0B0F12] rounded-lg border border-slate-800">
        ...
        {/* Officials */}
        <div className="space-y-3 bg-[#171D22] p-4 rounded-xl border border-slate-800/90 shadow-md">
          <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2">
            <UserCheck className="w-4 h-4 text-emerald-400" /> Match Officials
          </h4>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
            <div className="p-3 bg-[#0B0F12] rounded-lg border border-slate-800">
              <span className="text-slate-500 block text-[10px] uppercase font-semibold">Center Referee</span>
              <span className="font-bold text-white">{currentUserName}</span>
            </div>
            <div className="p-3 bg-[#0B0F12] rounded-lg border border-slate-800">
              <span className="text-slate-500 block text-[10px] uppercase font-semibold">Assistant Referee 1</span>
              <span className="font-bold text-white">Assistant Official 1</span>
            </div>
            <div className="p-3 bg-[#0B0F12] rounded-lg border border-slate-800">
              <span className="text-slate-500 block text-[10px] uppercase font-semibold">Assistant Referee 2</span>
              <span className="font-bold text-white">Assistant Official 2</span>
            </div>
            <div className="p-3 bg-[#0B0F12] rounded-lg border border-slate-800">
              <span className="text-slate-500 block text-[10px] uppercase font-semibold">4th Official</span>
