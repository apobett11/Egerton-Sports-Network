          ...
          <Card title="Settings">
            <nav className="space-y-1">
              {[
                ...
                { id: 'notifications', label: 'Notifications', icon: <Bell className="w-4 h-4 text-emerald-400" /> },
                { id: 'theme', label: 'Display & Theme', icon: <Moon className="w-4 h-4 text-blue-400" /> },
                { id: 'about', label: 'About System', icon: <Info className="w-4 h-4 text-gray-400" /> },
              ].map((item) => (
                <button
                  key={item.id}
                  type="button"
        ...
        <div className="md:col-span-8 lg:col-span-9 space-y-6">
            ...
            <Card title="Personal Profile Settings" subtitle="Update your contact details and personal referee profile information">
              <form onSubmit={handleSaveProfile} className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-300 mb-1">Official Role</label>
                    <div className="p-2.5 rounded-xl bg-[#111111] border border-[#2A2A2A] text-xs text-emerald-400 font-bold flex items-center justify-between">
                      <span>{profileData.role}</span>
                      <span className="px-2.5 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider text-emerald-400 bg-emerald-950/80 border border-emerald-500/30">READ-ONLY</span>
                    </div>
                  </div>
                </div>
            ...
            <Card title="Account Overview" subtitle="Read-only account metadata and registration info">
              <div className="space-y-4 text-xs">
                <div className="p-4 bg-[#111111] rounded-xl border border-[#2A2A2A] space-y-2">
                  ...
                  <div className="flex justify-between py-1 border-b border-[#2A2A2A]">
                    <span className="text-gray-400">Assigned Badge:</span>
                  ...
                  <div className="flex justify-between py-1">
                    ...
                    <span className="px-2.5 py-0.5 rounded-md text-[10px] font-black uppercase tracking-wider text-emerald-400 bg-emerald-950/80 border border-emerald-500/30">OFFICIALLY VERIFIED</span>
                  </div>
                </div>
              </div>
            </Card>
          ...
          {activeSection === 'notifications' && (
            <Card title="Notification Preferences" subtitle="Manage alerts for match assignments">
              <div className="space-y-4 text-xs">
                <div className="flex items-center justify-between p-4 bg-[#111111] rounded-xl border border-[#2A2A2A]">
                  <div>
                    <span className="font-bold text-white block">Email Assignment Alerts</span>
                    <span className="text-gray-400">Receive email notification when new matches are assigned</span>
                  </div>
                  <input
                    type="checkbox"
                ...
                <div className="flex items-center justify-between p-4 bg-[#111111] rounded-xl border border-[#2A2A2A]">
                  <div>
                    ...
                    <span className="text-gray-400">Receive SMS reminder 2 hours prior to kickoff</span>
                  </div>
                  <input
                    type="checkbox"
              ...
              <div className="space-y-4 text-xs">
                <div className="flex items-center justify-between p-4 bg-[#111111] rounded-xl border border-[#2A2A2A]">
                  <div>
                    ...
                    <span className="text-gray-400">High-contrast dark theme optimized for pitch-side viewing</span>
                  </div>
                  <input
                    type="checkbox"
