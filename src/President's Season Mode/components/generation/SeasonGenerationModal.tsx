...
export const SeasonGenerationModal: React.FC<SeasonGenerationModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto bg-black/70 backdrop-blur-xs transition-opacity">
      <div
        className={`w-full max-w-4xl rounded-xl border border-[#1a2e45] shadow-2xl overflow-hidden flex flex-col max-h-[90vh] ${
          isDark
            ? 'bg-[#0e1e2d] text-slate-100 shadow-black/80'
            : 'bg-white border-slate-200 text-slate-800 shadow-slate-300/50'
        } transition-all duration-300 animate-in fade-in zoom-in-95`}
      >
        {/* MODAL HEADER BAR */}
        <div
          className={`px-6 py-4 border-b border-[#1a2e45] flex items-center justify-between shrink-0 ${
            isDark ? 'bg-[#0e1e2d]' : 'bg-slate-50'
          }`}
        >
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xs bg-[#ff0046]/10 text-[#ff0046] flex items-center justify-center font-bold">
          ...
          <button
            ...
            className="p-2 rounded-xs text-slate-400 hover:text-white hover:bg-[#152a40] cursor-pointer min-h-[44px] min-w-[44px] flex items-center justify-center transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          {step === 'INITIAL_WARNING' && (
            <div className="space-y-6 py-4">
              <div className="p-6 rounded-sm bg-[#ff0046]/10 border border-[#ff0046]/25 space-y-4">
                <div className="flex items-center gap-3 text-[#ff0046] font-extrabold text-base">
              ...
              >
                ...
                <ul className="text-xs text-slate-400 space-y-2">
                  <li className="flex items-center gap-2">
                    <span className="w-5 h-5 rounded-full bg-[#ff0046]/15 text-[#ff0046] text-[10px] font-black flex items-center justify-center">1</span>
                    Confirm official pitch grounds
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="w-5 h-5 rounded-full bg-[#ff0046]/15 text-[#ff0046] text-[10px] font-black flex items-center justify-center">2</span>
                    Confirm available center referees pool
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="w-5 h-5 rounded-full bg-[#ff0046]/15 text-[#ff0046] text-[10px] font-black flex items-center justify-center">3</span>
                    Review participating teams across both divisions
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="w-5 h-5 rounded-full bg-[#ff0046]/15 text-[#ff0046] text-[10px] font-black flex items-center justify-center">4</span>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3 max-h-80 overflow-y-auto pr-1">
                {availablePitches.map((pitch) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xs bg-[#15273b] text-slate-300 flex items-center justify-center">
                    ...
                    <span className="px-2.5 py-1 rounded-xs text-[10px] font-black uppercase tracking-wider bg-[#00b04f]/15 text-[#00b04f] border border-[#00b04f]/30">
                      ✓ Available
                    </span>
                  </div>
                ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  <div className="flex items-center justify-between border-b border-[#1a2e45] pb-2">
                    <div className="flex items-center gap-2 text-[#ff0046] font-extrabold text-xs">
                  ...
                  <div className="space-y-1.5 max-h-48 overflow-y-auto text-xs">
                    {premierLeagueTeams.map((t) => (
                      <div key={t.id} className="flex items-center justify-between py-1 px-2 rounded-xs hover:bg-[#15273b]">
          ...
          {step === 'FIXTURE_PREVIEW' && generationResult && (
            <div className="space-y-6">
              ...
              <div
                className={`p-4 rounded-sm border flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 ${
                  generationResult.validation.isValid
                    ? 'bg-[#00b04f]/10 border-[#00b04f]/25 text-[#00b04f]'
                    : 'bg-[#ff0046]/10 border-[#ff0046]/25 text-[#ff0046]'
                }`}
              >
                <div className="flex items-center gap-3">
                  {generationResult.validation.isValid ? (
                    <CheckCircle2 className="w-5 h-5 text-[#00b04f] shrink-0" />
                  ) : (
                    <AlertCircle className="w-5 h-5 text-[#ff0046] shrink-0" />
                  )}
                  <div>
                ...
                <span className="px-3 py-1 rounded-xs text-[10px] font-black uppercase tracking-wider bg-white/10">
                  {generationResult.validation.isValid ? '✓ Schedule Verified' : 'Action Required'}
                </span>
              </div>

              {/* Competition Selector Tabs */}
              <div className="flex items-center gap-2 border-b border-[#1a2e45] pb-3">
                <button
                  onClick={() => setActivePreviewTab('EPL')}
                  className={`px-4 py-2 rounded-xs font-extrabold text-xs cursor-pointer transition-all ${
              ...
              {activePreviewTab === 'EPL' && generationResult.premierLeagueFixtures && (
                <div className="space-y-6 max-h-96 overflow-y-auto pr-1">
                  ...
                  <div className="space-y-3">
                    <h5 className="font-extrabold text-xs text-[#ff0046] uppercase tracking-wider flex items-center gap-2">
                    ...
                    {generationResult.premierLeagueFixtures.leg1_fixtures.map((leg) => (
                      <div
                        ...
                        className={`p-4 rounded-sm border space-y-2.5 ${
                          isDark ? 'bg-[#0e1c2b] border-[#1a2e45]' : 'bg-slate-50 border-slate-200'
                        }`}
                      >
                        <div className="font-extrabold text-xs text-slate-900 dark:text-white border-b border-[#1a2e45] pb-1.5 flex items-center justify-between">
                  ...
                  {/* Leg 2 Section */}
                  <div className="space-y-3 pt-4 border-t border-[#1a2e45]">
                    <h5 className="font-extrabold text-xs text-[#ff0046] uppercase tracking-wider flex items-center gap-2">
                      <Calendar className="w-4 h-4" />
              ...
              {activePreviewTab === 'CHAMPIONSHIP' && generationResult.championshipFixtures && (
                <div className="space-y-6 max-h-96 overflow-y-auto pr-1">
                  {/* Leg 1 Section */}
                  <div className="space-y-3">
                    <h5 className="font-extrabold text-xs text-sky-400 uppercase tracking-wider flex items-center gap-2">
                      <Calendar className="w-4 h-4" />
                      <span>Leg 1 Fixtures Schedule</span>
                    </h5>
          ...
          {step === 'SAVE_RESULT' && saveOutcome && (
            <div className="space-y-6 py-4">
              {saveOutcome.success ? (
                <div className="p-6 rounded-sm bg-[#00b04f]/10 border border-[#00b04f]/25 text-center space-y-4">
                  <div className="w-12 h-12 rounded-full bg-[#00b04f] text-white flex items-center justify-center mx-auto shadow-md">
                  ...
                  <div className="grid grid-cols-3 gap-3 pt-2">
                    <div className={`p-3 rounded-xs border ${isDark ? 'bg-[#0e1c2b] border-[#1a2e45]' : 'bg-white border-slate-200'}`}>
                      <div className="text-lg font-black text-[#00b04f]">{saveOutcome.eplCount}</div>
                      <div className="text-[10px] text-slate-400 font-bold">EPL Fixtures</div>
                    </div>
                    <div className={`p-3 rounded-xs border ${isDark ? 'bg-[#0e1c2b] border-[#1a2e45]' : 'bg-white border-slate-200'}`}>
                      <div className="text-lg font-black text-sky-400">{saveOutcome.champCount}</div>
                      <div className="text-[10px] text-slate-400 font-bold">Championship</div>
                    </div>
                    <div className={`p-3 rounded-xs border ${isDark ? 'bg-[#0e1c2b] border-[#1a2e45]' : 'bg-white border-slate-200'}`}>
                      <div className="text-lg font-black text-white">{saveOutcome.count}</div>
                      <div className="text-[10px] text-slate-400 font-bold">Total Saved</div>
                    </div>
                  </div>
                </div>
              ) : (
                <div className="p-6 rounded-sm bg-[#ff0046]/10 border border-[#ff0046]/25 text-center space-y-4">
                  <div className="w-12 h-12 rounded-full bg-[#ff0046] text-white flex items-center justify-center mx-auto shadow-md">
                  ...
                  {saveOutcome.error && (
                    <div className="p-3 rounded-xs bg-[#0e1c2b] border border-[#1a2e45] text-[11px] font-mono text-[#ff0046] text-left overflow-x-auto max-h-24">
                      {saveOutcome.error}
                    </div>
                  )}
                </div>
        ...
        <div
          className={`px-6 py-4 border-t border-[#1a2e45] flex items-center justify-between shrink-0 ${
            isDark ? 'bg-[#0e1e2d]' : 'bg-slate-50'
          }`}
        >
          {step === 'INITIAL_WARNING' && (
            <>
              <button
                onClick={onClose}
                className="px-4 py-2.5 rounded-xs border border-[#1a2e45] text-xs font-extrabold text-slate-400 hover:text-white hover:bg-[#152a40] cursor-pointer min-h-[44px] transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={() => setStep('CONFIRM_PITCHES')}
                className="flex items-center gap-2 px-5 py-2.5 rounded-xs bg-[#ff0046] hover:bg-[#e0003e] text-white font-black text-xs cursor-pointer min-h-[44px] transition-colors shadow-md"
              >
          ...
          {step === 'CONFIRM_TEAMS' && (
            <>
              ...
              <button
                ...
                className="flex items-center gap-2 px-5 py-2.5 rounded-xs bg-[#ff0046] hover:bg-[#e0003e] text-white font-black text-xs cursor-pointer min-h-[44px] shadow-md transition-colors"
              >
                <span>Generate Fixtures</span>
          ...
          {step === 'FIXTURE_PREVIEW' && (
            <>
              ...
              <button
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-xs font-black text-xs cursor-pointer min-h-[44px] transition-colors ${
                  generationResult?.validation.isValid
                    ? 'bg-[#ff0046] hover:bg-[#e0003e] text-white shadow-md'
                    : 'bg-[#15273b] text-slate-500 cursor-not-allowed opacity-60 border border-[#1a2e45]'
                }`}
              >
          ...
          {step === 'FINAL_CONFIRMATION' && (
            <>
              ...
              <button
                ...
                disabled={isSaving}
                className="flex items-center gap-2 px-6 py-2.5 rounded-xs bg-[#ff0046] hover:bg-[#e0003e] text-white font-black text-xs cursor-pointer min-h-[44px] shadow-md transition-colors"
              >
                {isSaving ? (
                  <>
          ...
          {step === 'SAVE_RESULT' && (
            <div className="w-full flex justify-end">
              <button
                onClick={onClose}
                className="px-6 py-2.5 rounded-xs bg-[#ff0046] hover:bg-[#e0003e] text-white font-black text-xs cursor-pointer min-h-[44px] transition-colors shadow-md"
              >
                Close & View Schedule
              </button>
            </div>
