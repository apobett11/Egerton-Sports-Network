-- Migration 42: Referee Match End & Walkover Alignment
-- Enforces:
-- 1. In a walkover (3-0), 3 goals awarded to winner, 0 to loser in league standings.
-- 2. Strictly ZERO player goal/assist attribution and ZERO goalkeeper clean sheets for walkovers.
-- 3. Complete table synchronization (fixtures, match_reports, match_events, canonical_permanent_results, league_standings, team_form, player_stats).
-- 4. Strict idempotency and zero double-counting via stats_processed guard.
...
BEGIN
    ...
    -- Detect walkover outcome from canonical results or official reports
    SELECT EXISTS (
        SELECT 1 FROM public.canonical_permanent_results
        WHERE match_uid = NEW.id AND UPPER(outcome) = 'WALKOVER'
    ) OR EXISTS (
